...
namespace enum_meta {
  ...
  constexpr std::string_view type_name() {
    ...
    if constexpr (key_enum_idx != std::string_view::npos) {
      return complete_name.substr(key_enum_idx + key_enum.length(),
                                  name_length - key_enum.length());
    }
    ...
    if constexpr (key_struct_idx != std::string_view::npos) {
      return complete_name.substr(key_struct_idx + key_struct.length(),
                                  name_length - key_struct.length());
    }
